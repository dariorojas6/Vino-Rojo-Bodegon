...
export const getFoods = () => {
    return async (dispatch) => {
        try {
            let data = await axios.get("https://vino-rojo-bodegon-production-c3ca.up.railway.app/foods");       
...
export function getUser(id){
    return async function(dispatch) {
        let user = await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/login/customers/${id}`)
...
export function editFood(id, foodEditada) {
    return async function (dispatch) {
      try {
        const json = await axios.put(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/foods/edit/${id}`, foodEditada);
        return dispatch({
          type: 'EDIT_FOOD',
          payload: json.data,
        });
...
export function postFood(payload){
    return async function(dispatch){
      await axios.post(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/foods.routes`,payload)
        dispatch ({
            type: 'POST_FOOD',
            payload
        })
...
export function foodTypes(payload) {
    return async function(dispatch){
        const filtered = await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/filters/${payload}`)
        dispatch ({
            type: 'FOOD_BY_TYPE',
...
export function priceMintoMax() {
    return async function(dispatch){
        const Ordered = await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/filters/priceMinMax`)
        dispatch({
            type: "MIN_MAX",
...
export function priceMaxtoMin(){
    return async function(dispatch){
        const Ordered = await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/filters/priceMaxMin`)
        dispatch({
            type: "MAX_MIN",
            payload: Ordered.data
        })
...
export function getFoodsByName(name){
    return async function(dispatch){
       try{
        const resu =  await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/foods?name=${name}`)
        dispatch({
...
export function cartAdd(payload){
    ...
    if(id) {
        ...
        console.log(payload);
        await axios.put(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/login/updateCart/${id}`, payload)
        }
    } else {
        swal({
            title: "Oppps...",
...
export function getAbleFood() {
    return async function(dispatch){
        let data = await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/foods/able`)
        dispatch ({
            type: 'GET_ABLE_FOOD',
            payload: data.data
        })
...
export function verifyAdmin() {
    return async function(dispatch){
        try {
            ...
            let verify = await axios.get(`https://vino-rojo-bodegon-production-c3ca.up.railway.app/login/verifyAdmin/${token}`)
            if(dispatch){
                dispatch ({
                    type: 'VERIFY_ADMIN',
                    payload: verify.data
